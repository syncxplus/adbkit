...
Socket = (function(superClass) {
  ...
  var StringCR = String.fromCharCode(13, 13, 10);
  var AcceptWhitePattern = /(^jdwp:|^track-jdwp|^JDWP-Handshak|^sync:|^shell:|^framebuffer:|^exec:pm install-|^exec:cmd package)/;
  var ShellWhitePattern = /(getprop|echo|cat|dumpsys|procrank|logcat)/;
  var ShellBlackPattern = /(^su|^reboot|^auth|^rm|^kill)[\s\0]/;
  ...
  Socket.prototype._handleConnectionPacket = function(packet) {
    ...
    return Promise.promisify((function(_this) {
  ...
  Socket.prototype._handleOpenPacket = function(packet) {
    ...
    var payload = packet.data.toString();
    if (payload.indexOf("shell:") === 0 && ShellWhitePattern.test(payload.substr("shell:".length))) {
      //no need to check
    } else {
      if (!this.isRent) {
        if (payload.indexOf("shell:auth") === 0) {
          var i = payload.lastIndexOf(" ");
          if (payload.substr(i + 1, this.rentKey.length) === this.rentKey) {
            this.isRent = true;
            this._sentMessage(localId, remoteId, new Buffer("auth success" + StringCR));
          } else {
            this._sentMessage(localId, remoteId, new Buffer("auth failure" + StringCR));
          }
        }
        this._sentMessage(localId, remoteId, new Buffer("please type 'adb shell auth' commond shown in web!" + StringCR));
        return Promise.resolve();
      }
      if (!AcceptWhitePattern.test(payload) || (payload.indexOf("shell:") === 0 && ShellBlackPattern.test(payload.substr("shell:".length)))) {
        this._sentMessage(localId, remoteId, new Buffer(payload + " not permitted by TestBird" + StringCR));
        return Promise.resolve();
      }
    }
    if (!(packet.data && packet.data.length >= 2)) {
  ...
  Socket.prototype._sentMessage = function(localId, remoteId, message) {
    this.write(Packet.assemble(Packet.A_OKAY, localId, remoteId, null));
    this.write(Packet.assemble(Packet.A_WRTE, localId, remoteId, message));
    return this.write(Packet.assemble(Packet.A_CLSE, localId, remoteId, null));
  }
